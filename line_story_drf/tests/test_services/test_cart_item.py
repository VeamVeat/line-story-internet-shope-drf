...
class TestCartItemService:
    ...
    def __is_product_in_cart_item_by_user(cart_item):
        product_exist = CartItem.objects.filter(user=cart_item.user, product_id=cart_item.product.id).exists()
        return product_exist

    @staticmethod
    def __is_product_in_cart_item(cart_item):
        product_exist = CartItem.objects.filter(user=cart_item.user).exists()
        return product_exist
    ...
    def test_delete_product(self, get_client_of_cart_item):
        ...
        product_exist = self.__is_product_in_cart_item_by_user(cart_item)
